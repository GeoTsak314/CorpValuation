...
- Πλήρης μετάβαση σε **signed values**:
  - οι τιμές αποθηκεύονται όπως τις πληκτρολογεί ο χρήστης
  - δεν γίνεται πλέον αυτόματη μετατροπή εξόδων / ζημιών / αποσβέσεων σε θετικά ποσά
- Οι υπολογισμοί ακολουθούν τη λογική του Excel template:
  - signed inputs
  - explicit `+` / `-` πράξεις στους τύπους
...
- Υποστήριξη αρνητικών ποσών όπου χρειάζεται, π.χ.:
  - Φόροι Εισοδήματος
  - αρνητικά αποτελέσματα
  - διορθωτικές εγγραφές
- Βελτιωμένοι τύποι δεικτών επιχειρηματικής αποδοτικότητας σύμφωνα με το νεότερο template.
- Διατηρείται το **formula-safe Excel import** για απλά formulas.
- Το **Asset Turnover** εμφανίζεται με **4 δεκαδικά**.
- Το Report tab εμφανίζει τα **τελευταία 3 διαθέσιμα έτη**.
...
- `app591.py`: κύριο αρχείο εφαρμογής
...
Η εφαρμογή περιλαμβάνει fallback μηχανισμό για απλά formulas.
...
```bat
pyinstaller --windowed app591.py
```

### Recommended stable distribution

```bat
pyinstaller --onedir --windowed app591.py
```
...
- Οι τιμές αποθηκεύονται όπως εισάγονται από τον χρήστη ή το Excel.
- Δεν γίνεται automatic `abs()` conversion.
- Οι υπολογισμοί ακολουθούν explicit formula flow σύμφωνα με το template.
